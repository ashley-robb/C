...
// nodes is 1 more than larger height of node's two subtrees.
{
	...
	if (root != NULL){ // recursive heigh function
        int lh = height (root->leftChild); // left height
        int rh = height (root->rightChild); // right height
        nodeheight = 1 + intmax (lh, rh); // taking the height that is greater and adding 1 to determine tree height
		}
	return nodeheight;
...
//	p	- points to parent of node to be deleted.
{
	...
	if (n->leftChild != NULL) {	// there is left child
        ...
        delete(temp,n); // recursive function call
        ...
        if (temp != NULL) { // if not null then there is a parent
            if (temp->leftChild == n) temp->leftChild = NULL; // check for left delete
            else if (temp->rightChild == n) temp->rightChild = NULL; // check for right delete
        }
        free(n);
		}
